...
Device::State Audio_Win32::state() {
	return this->stat;
}

Awaitable<> Audio_Win32::stateChange(int waitFlags) {
...
Audio_Win32::Buffer::Buffer(Audio_Win32 &device, int capacity)
	: BufferImpl(new uint8_t[capacity], capacity, device.stat)
	, device(device)
{
...
bool Audio_Win32::Buffer::start(Op op) {
	...
	if (this->device.stat == Device::State::READY)
		start();
